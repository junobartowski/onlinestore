...
class AccountDetailsForm extends CFormModel {
...
//Start of ajax live checking
    public function getEmailAddressByEmailAddress($emailAddress) {
        ...
        $sql = "SELECT EmailAddress FROM accountdetails WHERE EmailAddress = :emailAddress";
...
//End of ajax live checking
    public function isEmailAddressExisting($emailAddress) {
        $connection = $this->_connection;
        $sql = "SELECT COUNT(ID) ctrID FROM accountdetails WHERE EmailAddress = :emailAddress";
        $command = $connection->createCommand($sql);
        $command->bindValues(array(":emailAddress" => $emailAddress));
        $result = $command->queryRow();
    ...
    public function isMobileNumberExisting($mobileNumber) {
        $connection = $this->_connection;
        $sql = "SELECT COUNT(ID) ctrID FROM accountdetails WHERE MobileNumber = :mobileNumber";
        $command = $connection->createCommand($sql);
    ...
    public function isMobileNumberAndCountryCodeMatched($mobileNumber, $countryCode) {
        $connection = $this->_connection;
        $sql = "SELECT COUNT(ID) ctrID FROM accountdetails WHERE MobileNumber = :mobileNumber AND CountryCode = :countryCode";
        $command = $connection->createCommand($sql);
    ...
    public function getAdministratorEmailAddress() {
        ...
        $connection = $this->_connection;
        $sql = "SELECT ad.EmailAddress FROM accountdetails ad
    ...
    public function getFirstNameByAccountID($accountID) {
        $connection = $this->_connection;
        $sql = "SELECT FirstName FROM accountdetails WHERE AccountID = :accountID";
        $command = $connection->createCommand($sql);
